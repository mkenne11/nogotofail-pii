...
public class HttpsPiiTest extends BackgroundTestForHttpPii {
    ...
    URL url;
    final int NUM_TEST_ITERATIONS = 2;

    protected HttpsPiiTest(Context app_context) {
        super(app_context);
    }
    /**
    ...
    protected void RunPiiQueryStringTest() throws Exception {
        try {
            ...
            url = new URL(HTTPS_TARGET + "?google_ad_id=" + google_ad_id);
            connection = (HttpURLConnection) url.openConnection();
            connection.setConnectTimeout(CONNECTION_TIMEOUT);
            connection.setReadTimeout(CONNECTION_TIMEOUT);
            setProgressMessage("Issuing HTTPS request with PII ID in query string");
            setTestResult(connection.getResponseCode() + " " + connection.getResponseMessage());
            connection.disconnect();

            // Send request with PII location in query string
            url = new URL(HTTPS_TARGET + "?longtitude=" + location_longitude +
                    "&latitude=" + location_latitude);
            connection = (HttpURLConnection) url.openConnection();
            connection.setConnectTimeout(CONNECTION_TIMEOUT);
            connection.setReadTimeout(CONNECTION_TIMEOUT);
            setProgressMessage("Issuing HTTPS request with PII location in query string");
            setTestResult(connection.getResponseCode() + " " + connection.getResponseMessage());
            connection.disconnect();

            // Send request with PII details in query string
            url = new URL(HTTPS_TARGET + "?email=" + email);
            connection = (HttpURLConnection) url.openConnection();
            connection.setConnectTimeout(CONNECTION_TIMEOUT);
            connection.setReadTimeout(CONNECTION_TIMEOUT);
            setProgressMessage("Issuing HTTPS request with PII detail in query string");
            setTestResult(connection.getResponseCode() + " " + connection.getResponseMessage());
            connection.disconnect();
        } catch (Exception e) {
    ...
    protected void RunPiiHeaderTest() throws Exception {
        try {
            ...
            url = new URL(HTTPS_TARGET);
            connection = (HttpURLConnection) url.openConnection();
            connection.setConnectTimeout(CONNECTION_TIMEOUT);
            connection.setReadTimeout(CONNECTION_TIMEOUT);
            connection.setRequestProperty("Header-Identifier", google_ad_id);
            setProgressMessage("Issuing HTTPS request with PII ID in header");
            setTestResult(connection.getResponseCode() + " " + connection.getResponseMessage());
            connection.disconnect();
            ...
            setProgressMessage("Issuing HTTPS request with PII location in header");
            setTestResult(connection.getResponseCode() + " " + connection.getResponseMessage());
            connection.disconnect();
            ...
            setProgressMessage("Issuing HTTPS request with PII details in header");
            setTestResult(connection.getResponseCode() + " " + connection.getResponseMessage());
            connection.disconnect();
        }
        finally {
    ...
    protected void RunPiiMessageBodyTest() throws Exception {
        ...
        try {
            HttpPost post = new HttpPost(HTTPS_TARGET);
            json_data = new JSONObject();
            json_data.put("google_ad_id", google_ad_id);
            StringEntity se = new StringEntity(json_data.toString());
            se.setContentType(new BasicHeader(HTTP.CONTENT_TYPE, "application/json"));
            post.setEntity(se);
            setProgressMessage("Issuing HTTPS request with identifiers in message body");
        ...
        try {
            ...
            se.setContentType(new BasicHeader(HTTP.CONTENT_TYPE, "application/json"));
            post.setEntity(se);
            setProgressMessage("Issuing HTTPS request with PII location in message body");
            response = client.execute(post);
        ...
        try {
            ...
            se.setContentType(new BasicHeader(HTTP.CONTENT_TYPE, "application/json"));
            post.setEntity(se);
            setProgressMessage("Issuing HTTPS request with PII details in message body");
            response = client.execute(post);
